...
<?php
$id = $_GET['id'];
$sql = "SELECT * FROM rajasthan_tour WHERE id = $id";
$result = $conn->query($sql);
$rajasthanrow = $result->fetch_assoc();
?>
...
<!-- Modern Hero Section -->
<section class="hero-section position-relative" style="height: 420px; background: linear-gradient(to bottom, rgba(0,0,0,0.3) 0%, rgba(0,0,0,0.7) 100%), url('admin/html/<?php echo htmlspecialchars($rajasthanrow['image']); ?>') center center/cover no-repeat;">
  <div class="container h-100 position-relative">
    <div class="row h-100 align-items-center">
      <div class="col-lg-8 col-md-10 col-12 mx-auto text-center text-white" style="z-index:2;">
        <h1 class="display-4 fw-bold mb-3" style="text-shadow:0 2px 16px rgba(0,0,0,0.4)"><?php echo htmlspecialchars($rajasthanrow['title']); ?></h1>
        <p class="lead mb-4" style="text-shadow:0 2px 8px rgba(0,0,0,0.3)">Explore the Rajasthan Tours with us.</p>
    ...
    <div class="row g-3 justify-content-center text-center">
      <div class="col-6 col-md-3">
        <div class="info-card-glass p-3 rounded-4 h-100 shadow-sm">
          ...
          <div class="fs-5 fw-semibold mt-1"><?php echo htmlspecialchars($rajasthanrow['duration']); ?></div>
        </div>
      </div>
      <div class="col-6 col-md-3">
        <div class="info-card-glass p-3 rounded-4 h-100 shadow-sm">
          ...
          <div class="fs-5 fw-semibold mt-1"><?php echo htmlspecialchars($rajasthanrow['persons']); ?></div>
        </div>
      </div>
      <div class="col-6 col-md-3">
        <div class="info-card-glass p-3 rounded-4 h-100 shadow-sm">
    ...
    <div class="row">
      ...
      <div class="col-lg-8 col-12 mb-4 mb-lg-0">
        <div class="card shadow-sm border-0 mb-4">
          <img style="height:350px; width:100%; object-fit:cover;" src="admin/html/<?php echo htmlspecialchars($rajasthanrow['image']); ?>" alt="Tour Image" class="card-img-top rounded-top">
          <div class="card-body">
            <h1 class="card-title mb-3"><?php echo htmlspecialchars($rajasthanrow['title']); ?></h1>
            <div class="d-flex flex-wrap mb-3">
              <div class="me-4 mb-2"><i class="fa fa-clock text-primary me-1"></i> <?php echo htmlspecialchars($rajasthanrow['duration']); ?></div>
              <div class="me-4 mb-2"><i class="fa fa-user text-primary me-1"></i> <?php echo htmlspecialchars($rajasthanrow['persons']); ?></div>
              <div class="mb-2"><i class="fa fa-map-marker-alt text-primary me-1"></i> <?php echo htmlspecialchars($rajasthanrow['places_covered']); ?></div>
            </div>
            <p class="card-text mb-3"><?php echo nl2br(htmlspecialchars($rajasthanrow['description'])); ?></p>
          ...
          <div class="card-body">
            ...
            <div class="accordion" id="customItinerary">
              ...
              for ($i = 1; $i <= 17; $i++) {
                ...
                if (!empty($rajasthanrow[$headingKey])) {
                  ...
                  echo '<div class="fw-semibold text-dark">' . htmlspecialchars($rajasthanrow[$headingKey]) . '</div>';
                  echo '</button>';
                  echo '</h2>';
                  echo '<div id="collapse' . $i . '" class="accordion-collapse collapse ' . ($i === 1 ? 'show' : '') . '" data-bs-parent="#customItinerary">';
                  echo '<div class="accordion-body">' . nl2br(htmlspecialchars($rajasthanrow[$descKey])) . '</div>';
                  echo '</div>';
                  echo '</div>';
                }
              }
        ...
        <div class="sticky-top" style="top: 100px;">
          <div class="card shadow-sm mb-4">
            ...
            <ul class="list-group list-group-flush">
              <?php
              $sql = "SELECT id, title, image, duration FROM rajasthan_tour WHERE id != $id ORDER BY RAND() LIMIT 4";
              $result = $conn->query($sql);
              while ($row = $result->fetch_assoc()) {
                ...
                echo '<div>';
                echo '<a href="rajasthan-detail-page.php?id=' . $row['id'] . '" class="fw-bold text-decoration-none text-dark">' . htmlspecialchars($row['title']) . '</a><br>';
...
<!-- Carousel of Other Rajasthan Packages -->
<section class="py-5 bg-light">
  <div class="container">
    <h2 class="mb-4 text-center">Other Rajasthan Packages</h2>
    <div class="owl-carousel owl-theme">
      <?php
      $sql = "SELECT id, title, image, duration, persons, places_covered, description FROM rajasthan_tour WHERE id != $id ORDER BY RAND() LIMIT 8";
      $result = $conn->query($sql);
      while ($row = $result->fetch_assoc()) {
        ...
        echo '<a href="rajasthan-detail-page.php?id=' . $row['id'] . '" class="btn btn-outline-primary btn-sm mt-2">View Details</a>';
        echo '</div>';
        echo '</div>';
        echo '</div>';
      }
