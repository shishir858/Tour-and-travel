...
if (isset($_GET['delete'])) {
    ...
    $conn->query("DELETE FROM himachal_packages WHERE id = $id");
    header('Location: himachal-packages.php');
    exit;
}
...
if(isset($_GET['ajax']) && $_GET['ajax'] == 'get' && isset($_GET['id'])) {
    $id = intval($_GET['id']);
    $result = $conn->query("SELECT * FROM himachal_packages WHERE id = $id");
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if(isset($_FILES['image']) && $_FILES['image']['error'] == 0) {
        $upload_dir = '../assets/img/himachal-tour-images/';
        if(!is_dir($upload_dir)) {
            mkdir($upload_dir, 0755, true);
        }
        $file_ext = strtolower(pathinfo($_FILES['image']['name'], PATHINFO_EXTENSION));
        $new_filename = 'himachal_' . time() . '_' . rand(1000,9999) . '.' . $file_ext;
        $target_file = $upload_dir . $new_filename;
        if(move_uploaded_file($_FILES['image']['tmp_name'], $target_file)) {
            $image = 'assets/img/himachal-tour-images/' . $new_filename;
            // Delete old image
            if($id > 0 && !empty($old_image) && file_exists(__DIR__ . '/../' . $old_image)) {
                unlink(__DIR__ . '/../' . $old_image);
            }
        }
    } elseif($id > 0 && empty($image)) {
        $result = $conn->query("SELECT image FROM himachal_packages WHERE id=$id");
        if($row = $result->fetch_assoc()) {
            $image = $row['image'];
        }
    }
    ...
    $gallery_images = handleGalleryUpload($_FILES, 'himachal_packages', $id);
    ...
    if ($id > 0) {
        $sql = "UPDATE himachal_packages SET 
        ...
        $conn->query("INSERT INTO himachal_packages ($cols) VALUES ($vals)");
    }
    header('Location: himachal-packages.php');
    exit;
}

$packages = $conn->query("SELECT * FROM himachal_packages ORDER BY id DESC");
?>
<!DOCTYPE html>
<html lang="en">
<head>
...
<body>
    ...
    <main id="main-content">
        <h1 class="page-title">Himachal Packages</h1>
            ...
            <div class="modal-content">
                <form method="POST" enctype="multipart/form-data">
                    <div class="modal-header">
                        <h5 class="modal-title">Himachal Package Form</h5>
