...
if (isset($_GET['delete'])) {
    ...
    $conn->query("DELETE FROM tajmahal_tours WHERE id = $id");
    header('Location: tajmahal-tours.php');
    exit;
}
...
if(isset($_GET['ajax']) && $_GET['ajax'] == 'get' && isset($_GET['id'])) {
    $id = intval($_GET['id']);
    $result = $conn->query("SELECT * FROM tajmahal_tours WHERE id = $id");
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if(isset($_FILES['image']) && $_FILES['image']['error'] == 0) {
        $upload_dir = '../assets/img/tajmahal/';
        if(!is_dir($upload_dir)) {
            mkdir($upload_dir, 0755, true);
        }
        $file_ext = strtolower(pathinfo($_FILES['image']['name'], PATHINFO_EXTENSION));
        $new_filename = 'tajmahal_' . time() . '_' . rand(1000,9999) . '.' . $file_ext;
        $target_file = $upload_dir . $new_filename;
        if(move_uploaded_file($_FILES['image']['tmp_name'], $target_file)) {
            $image = 'assets/img/tajmahal/' . $new_filename;
            // Delete old image
        ...
        $result = $conn->query("SELECT image FROM tajmahal_tours WHERE id=$id");
        if($row = $result->fetch_assoc()) {
            $image = $row['image'];
        }
    }
    ...
    $gallery_images = handleGalleryUpload($_FILES, 'tajmahal_tours', $id);
    ...
    if ($id > 0) {
        ...
        $conn->query("INSERT INTO tajmahal_tours ($cols) VALUES ($vals)");
    }
    header('Location: tajmahal-tours.php');
    exit;
}

$packages = $conn->query("SELECT * FROM tajmahal_tours ORDER BY id DESC");
?>
<!DOCTYPE html>
<html lang="en">
<head>
...
<body>
    ...
    <main id="main-content">
        <h1 class="page-title">Taj Mahal Tours</h1>
            ...
            <div class="modal-content">
                <form method="POST" enctype="multipart/form-data">
                    <div class="modal-header">
                        <h5 class="modal-title">Taj Mahal Tour Form</h5>
