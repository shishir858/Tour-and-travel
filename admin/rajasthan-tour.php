...
if (isset($_GET['delete'])) {
    ...
    $conn->query("DELETE FROM rajasthan_tour WHERE id = $id");
    header('Location: rajasthan-tour.php');
    exit;
}
...
if(isset($_GET['ajax']) && $_GET['ajax'] == 'get' && isset($_GET['id'])) {
    $id = intval($_GET['id']);
    $result = $conn->query("SELECT * FROM rajasthan_tour WHERE id = $id");
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if(isset($_FILES['image']) && $_FILES['image']['error'] == 0) {
        $upload_dir = '../assets/img/rajasthan/';
        if(!is_dir($upload_dir)) {
            mkdir($upload_dir, 0755, true);
        }
        $file_ext = strtolower(pathinfo($_FILES['image']['name'], PATHINFO_EXTENSION));
        $new_filename = 'rajasthan_' . time() . '_' . rand(1000,9999) . '.' . $file_ext;
        $target_file = $upload_dir . $new_filename;
        if(move_uploaded_file($_FILES['image']['tmp_name'], $target_file)) {
            $image = 'assets/img/rajasthan/' . $new_filename;
            // Delete old image
        ...
        $result = $conn->query("SELECT image FROM rajasthan_tour WHERE id=$id");
        if($row = $result->fetch_assoc()) {
            $image = $row['image'];
        }
    }
    ...
    $gallery_images = handleGalleryUpload($_FILES, 'rajasthan_tour', $id);
    
    // Build itinerary fields (18 sets for rajasthan_tour)
    $itinerary_fields = [];
    for($i = 1; $i <= 18; $i++) {
    ...
    if ($id > 0) {
        ...
        $conn->query("INSERT INTO rajasthan_tour ($cols) VALUES ($vals)");
    }
    header('Location: rajasthan-tour.php');
    exit;
}

$packages = $conn->query("SELECT * FROM rajasthan_tour ORDER BY id DESC");
?>
<!DOCTYPE html>
<html lang="en">
<head>
...
<body>
    ...
    <main id="main-content">
        <h1 class="page-title">Rajasthan Tours</h1>
        
        <div class="dashboard-card mb-4">
            <button class="btn btn-primary" data-bs-toggle="modal" data-bs-target="#packageModal" onclick="resetForm()">Add New Tour</button>
        </div>
            ...
            <div class="modal-content">
                <form method="POST" enctype="multipart/form-data">
                    <div class="modal-header">
                        <h5 class="modal-title">Rajasthan Tour Form</h5>
