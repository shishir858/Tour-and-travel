...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $query = "INSERT INTO tour_packages_row2 (package_name, package_image, description, rating, booking_link)
              VALUES ('$package_name', '$target_file', '$description', '$rating', '$booking_link')";
                ...
                <div class="container-fluid">
                    <form action="" method="POST" enctype="multipart/form-data">
                        ...
                        <div class="mb-3">
                            ...
                            <input type="text" class="form-control" id="booking_link" name="booking_link" required>
                        </div>
            ...
            <div class="container-fluid">
                ...
                <div class="table-responsive">
                    <table class="table table-bordered w-100">
                        ...
                        <tbody>
                            <?php
                            $result = mysqli_query($conn, "SELECT * FROM tour_packages_row2 ORDER BY id DESC");
                            while ($row = mysqli_fetch_assoc($result)) { ?>
                                <tr>
                                    <td><img src="<?= $row['package_image']; ?>" width="100"></td>
                                    <td><?= htmlspecialchars($row['package_name']); ?></td>
                                    <td><?= htmlspecialchars($row['description']); ?></td>
                                    <td><?= str_repeat("⭐", $row['rating']); ?></td>
                                    <td><a href="<?= htmlspecialchars($row['booking_link']); ?>" target="_blank">View</a>
                                    </td>
                                    <td class="text-center" style="white-space: nowrap;">
                                        <a href="edit-package2.php?edit_id=<?= $row['id']; ?>"
                                            class="btn btn-warning btn-sm">Edit</a>
                                        <a href="delete-package2.php?delete_id=<?= $row['id']; ?>"
