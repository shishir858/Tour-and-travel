...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    // $package_image = mysqli_real_escape_string($conn, $_POST['package_image']);
    ...
    $query = "INSERT INTO `other_services` (`category`,`package_name`, `package_image`, `description`) 
    VALUES ('$category','$package_name','$target_file', '$description')";
            ...
            <div class="container-fluid">
                ...
                <div class="table-responsive">
                    <table class="table table-bordered w-100">
                        ...
                        <tbody>
                            <?php
                            $result = mysqli_query($conn, "SELECT * FROM other_services ORDER BY id DESC");
                            while ($row = mysqli_fetch_assoc($result)) { ?>
                                <tr>
                                    <td><img style="height:150px;width:100px;object-fit:cover;" src="<?= $row['package_image']; ?>" width="100"></td>
                                    <td><?= htmlspecialchars($row['category']); ?></td>
                                    <td><?= htmlspecialchars($row['package_name']); ?></td>
                                    <td><?= htmlspecialchars($row['package_image']); ?></td>
                                    <td><?= htmlspecialchars($row['description']); ?></td>
                                   
                                  
                                    <td class="text-center" style="white-space: nowrap;">
                                        <a href="edit-other-services.php?edit_id=<?= $row['id']; ?>"
                                            class="btn btn-warning btn-sm">Edit</a>
                                        <a href="delete-other-services.php?delete_id=<?= $row['id']; ?>"
